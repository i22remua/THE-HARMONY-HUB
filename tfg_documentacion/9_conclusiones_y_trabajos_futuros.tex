...
\begin{tabularx}{\textwidth}{|P{5.7cm}|Y|P{3.1cm}|}
\hline
\textbf{Objetivo} & \textbf{Evidencia de cumplimiento} & \textbf{Estado} \\
\hline
Investigar y diseñar modelos de aprendizaje automático para relacionar el estado emocional con características musicales funcionales. & Se implementó un enfoque híbrido con heurísticas explícitas, aprendizaje individual del perfil y un clasificador supervisado de sesión con regresión logística. No se materializaron técnicas como \textit{reinforcement learning} o clustering en el entregable final, pero sí un modelo defendible, trazable y activado mediante puertas de datos, calidad y confianza operativa. & Cumplido con enfoque alternativo \\
\hline
Diseñar una interfaz de usuario móvil intuitiva y táctil para facilitar la evaluación diaria del estado de ánimo y la visualización del feedback. & La aplicación móvil implementa acceso, autenticación, check-in guiado, medición opcional del entorno, visualización de recomendación, envío de feedback e histórico básico. Las capturas del manual y las pruebas funcionales muestran un flujo usable y coherente de extremo a extremo. & Cumplido \\
\hline
Implementar la integración de servicios en la nube mediante API REST para alojar el motor de recomendación y la gestión de la base de datos musical y del feedback. & El backend en FastAPI expone endpoints para autenticación con Spotify, check-ins, recomendaciones, generación de playlists y feedback. Firestore persiste sesiones, playlists, señales de aprendizaje y catálogo estructurado, mientras que Spotify materializa el resultado final. & Cumplido \\
\hline
Desarrollar un módulo de procesamiento de señales que capture y clasifique el ruido ambiental en tiempo real utilizando las APIs del dispositivo móvil. & La app incorpora medición opcional del entorno acústico y el backend clasifica esa señal en categorías funcionales que pueden modificar el perfil objetivo y las consultas auxiliares. La solución no constituye un sistema avanzado de DSP clínico, pero sí una capa operativa de captura y aprovechamiento contextual del entorno en tiempo de uso. & Cumplido en el alcance del TFG \\
\hline
Implementar mecanismos de seguridad para la gestión de datos sensibles del usuario y para la comunicación entre la aplicación móvil y el backend en la nube. & El sistema integra autenticación de usuarios, separación entre datos públicos y privados, reglas de seguridad de Firestore, control de acceso por identificadores de usuario y gestión de credenciales fuera del repositorio. La comunicación con Spotify y con los servicios en la nube se apoya además en flujos autenticados y en la infraestructura segura habitual de Firebase y HTTPS. & Cumplido \\
\hline
\end{tabularx}
