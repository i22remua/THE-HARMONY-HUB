...
\begin{tabularx}{\textwidth}{|P{3.1cm}|Y|P{2.2cm}|}
\hline
\textbf{Objetivo} & \textbf{Evidencia de cumplimiento} & \textbf{Estado} \\
\hline
Diseñar una app capaz de recoger el contexto del usuario mediante un check-in breve y usable. & La aplicación móvil implementa acceso, check-in guiado, medición opcional del entorno y navegación completa de sesión. Las pruebas funcionales verifican el flujo de extremo a extremo. & Cumplido \\
\hline
Generar recomendaciones de sesión y playlists reales adaptadas al contexto. & El backend construye candidatos de modo, genera un perfil musical y materializa playlists reales en Spotify. Se validaron casos operativos en relajación, foco y energía. & Cumplido \\
\hline
Incorporar persistencia, historial y trazabilidad completa entre recomendación, playlist y feedback. & Firestore almacena sesiones, playlists, feedback y documentos auxiliares de aprendizaje. La traza funcional queda enlazada mediante identificadores de sesión y recomendación. & Cumplido \\
\hline
Integrar aprendizaje progresivo basado en feedback real. & El sistema mantiene aprendizaje individual visible mediante pesos contextuales y estables, además de estructuras agregadas en \texttt{user\_generation\_preferences} y ejemplos de entrenamiento a nivel de sesión. & Cumplido \\
\hline
Explorar un modelo supervisado defendible y prudente para apoyar la selección de modo. & Se implementó un pipeline con regresión logística, validación cruzada cuando es posible, puertas de datos y calidad, mantenimiento automático, umbral operativo de confianza y evidencias de activación real en sesiones finales de validación. & Cumplido \\
\hline
\end{tabularx}
...
El último objetivo puede considerarse cumplido porque el sistema ya no se
limita a implementar el pipeline, sino que demuestra activación real del
clasificador de sesión dentro del flujo operativo de la app. Aun así, conviene
subrayar que este cumplimiento no sustituye el carácter híbrido del sistema:
la recomendación sigue combinando heurística, aprendizaje individual y ajuste
supervisado de forma trazable.
...
Los metadatos finales del modelo recogen 65 ejemplos de sesión, con 53 útiles
